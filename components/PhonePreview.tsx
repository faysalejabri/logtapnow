...
const PhonePreview: React.FC<PhonePreviewProps> = ({ profile }) => {
  ...
  // Dynamic Styles based on user input
  const buttonStyle = {
        ...
        <div className="pt-14 px-4 pb-12 flex flex-col items-center relative z-10">
          ...
          <div className="w-full bg-white rounded-2xl shadow-xl overflow-hidden flex flex-row h-48 mb-6 transform transition-all hover:scale-[1.01] duration-300">
            ...
            <div className="w-[45%] relative bg-gray-200 h-full">
               {profile.profilePicture ? (
                ...
                <div 
                  className="absolute inset-0 flex items-center justify-center text-gray-400 bg-gray-200"
                >
                  <User size={32} />
                </div>
              )}
            ...
            <div className="w-[55%] p-4 flex flex-col justify-center text-left bg-white relative">
              <h2 className="text-xl font-black text-gray-900 leading-tight mb-2">
                ...
                <span className="block text-gray-600">{profile.lastName || 'Name'}</span>
              </h2>
              
              <div className="h-1 w-8 mb-3 rounded-full" style={{ backgroundColor: theme.primaryColor }}></div>
          ...
          {profile.bio && (
            <div className="mt-6 w-full bg-white p-4 rounded-xl shadow-sm border border-gray-100">
               <h3 className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-2">{t('about')}</h3>
          ...
          <div className="mt-6 w-full space-y-3">
             {profile.email && (
                 <a href={`mailto:${profile.email}`} className="flex items-center gap-3 p-3 bg-white rounded-xl shadow-sm border border-gray-100 hover:bg-gray-50 transition-colors">
                     <div className="p-2 rounded-lg bg-blue-50 text-blue-600">
                         <SOCIAL_ICONS.email size={16} />
                     </div>
                     <span className="text-sm text-gray-700 font-medium break-all">{profile.email}</span>
                 </a>
             )}
              {profile.phone && (
                 <a href={`tel:${profile.phone}`} className="flex items-center gap-3 p-3 bg-white rounded-xl shadow-sm border border-gray-100 hover:bg-gray-50 transition-colors">
          ...
          {profile.address && (
            <div className="mt-6 w-full bg-white p-1 rounded-xl shadow-sm border border-gray-100">
              ...
              <div className="p-3 flex items-start gap-2">
                 ...
                 <p className="text-xs text-gray-600 font-medium">{profile.address}</p>
              </div>
            </div>
          )}
